...
**Observações gerais**:
- Muitos testes usam mocks de `prisma/client` definidos em cada arquivo (e complementados pelo `src/__tests__/setup.ts`), o que permite testar regras de negócio sem dependência do banco real.
- Os utilitários `hash` e `jwt` são testados tanto isoladamente (em `utils`) quanto mockados localmente nos serviços quando necessário, garantindo cobertura dos dois níveis (integração local e unidade).
- Testes de serviços focam em fluxos felizes e em muitos caminhos de falha importantes (validações, conflitos, transações).

Se quiser, posso:
- Ajustar o nível de detalhe (ex.: incluir exemplos de inputs/outputs esperados para cada teste).
- Gerar um sumário em formato mais curto (tabela) ou adicionar links para os arquivos de implementação.
- Abrir um PR com este arquivo e um commit.

Quer que eu altere o formato ou já crie um commit com `docs/TESTS.md`?
