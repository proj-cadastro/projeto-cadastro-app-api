...
export async function create(req: Request, res: Response) {
  try {
    ...
    if (!existe)
      return res.status(400).json({sucesso: false,  mensagem: "Coordenador informado não existe" });

    const jaCoordenador = await professorService.isProfessorCoordenador(coordenadorId);
    if (jaCoordenador)
      return res.status(400).json({ sucesso: false, mensagem: "Professor já é coordenador de outro curso" });

    const curso = await cursoService.createCurso(req.body);
    return res.status(201).json({ sucesso: true, mensagem: "Curso criado com sucesso", data: curso });
  } catch (error) {
    return res.status(400).json({ sucesso: false, mensagem: "Erro ao criar curso", erro: (error as Error).message });
  }
}
...
export async function getAll(req: Request, res: Response) {
  try {
    ...
    if (!cursos || cursos.length === 0)
      return res.status(404).json({ sucesso: false, mensagem: 'Nenhum curso encontrado', data:[] });
    return res.status(200).json({ sucesso: true, mensagem: "Cursos encontrados com sucesso", data: cursos });
  } catch (error) {
    return res.status(400).json({ sucesso: false, mensagem: "Erro ao buscar cursos", erro: (error as Error).message });
  }
}
...
export async function getById(req: Request, res: Response) {
  try {
    ...
    return res.status(400).json({ sucesso: false, mensagem: "Erro ao buscar curso", erro: (error as Error).message });
  }
}
...
export async function update(req: Request, res: Response) {
  try {
    ...
    if (!cursoBuscado) return res.status(404).json({ sucesso: false, mensagem: 'Curso não encontrado' });
...
export async function remove(req: Request, res: Response) {
  try {
    ...
    return res.status(400).json({ sucesso: false, mensagem: "Erro ao deletar curso", erro: (error as Error).message });
  }
}
...
export async function hasMateriasExclusivas(req: Request, res: Response) {
  try {
    ...
    return res.status(400).json({ sucesso: false, mensagem: "Erro ao verificar matérias exclusivas", erro: (error as Error).message });
  }
}
